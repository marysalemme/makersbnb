class MakersBnb < Sinatra::Base
  ...
  helpers do
    ...
    def check_booking_errors
      ...
      booking_error("Space is occupied") if space_occupied?
    end
    ...
    def space_occupied?(start_date = params[:start_date], end_date = params[:end_date])
      space = Space.get(session[:space_id])
      space.occupied? (Date.parse(start_date)...Date.parse(end_date)).to_a
    end
  end
end
